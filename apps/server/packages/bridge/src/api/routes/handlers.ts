  ...
  try {
    ...
    modules.forEach((handlerModule) => {
      ...
      if (!actionName || !handlerModule.default || !handlerModule.validator) {
        logger.warn(
          `[Bridge] Skipping handler "${handlerModule.name}" due to missing exports`,
        );
        return;
      }

      const authZCheck = authZMiddleware
        .canExecuteActionOnResource(`bridge:${actionName}`)
    ...
    logger.info(`[Bridge] handlers loaded (${Date.now() - startTimer}ms)`);
  } catch (err) {
    logger.error(
      '[Bridge] Error loading handlers, check the generated index file:',
      err,
    );
  }
};
