...
function generateHandlersIndex() {
  ...
  try {
    ...
    console.log(
      `[Methods] Prebuild: Generated handlers index (${handlerFiles.length} files, ${Date.now() - startTimer}ms)`,
    );
  } catch (err) {
    console.error('[Methods] Prebuild: Error generating handlers index:', err);
    process.exit(1);
  }
}
