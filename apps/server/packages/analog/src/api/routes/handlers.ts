  ...
  try {
    ...
    modules.forEach((handlerModule) => {
      ...
      if (!actionName || !handlerModule.default || !handlerModule.validator) {
        logger.warn(
          `[Analog] Skipping handler "${handlerModule.name}" due to missing exports`,
        );
        return;
      }

      const authZCheck = authZMiddleware
        .canExecuteActionOnResource(`analog:${actionName}`)
    ...
    logger.info(`[Analog] handlers loaded (${Date.now() - startTimer}ms)`);
  } catch (err) {
    logger.error(
      '[Analog] Error loading handlers, check the generated index file:',
      err,
    );
  }
};
