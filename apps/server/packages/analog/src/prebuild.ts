...
function generateHandlersIndex() {
  ...
  try {
    ...
    console.log(
      `[Analog] Prebuild: Generated handlers index (${handlerFiles.length} files, ${Date.now() - startTimer}ms)`,
    );
  } catch (err) {
    console.error('[Analog] Prebuild: Error generating handlers index:', err);
    process.exit(1);
  }
}
