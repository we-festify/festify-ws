  ...
  try {
    ...
    modules.forEach((handlerModule) => {
      ...
      if (!actionName || !handlerModule.default || !handlerModule.validator) {
        logger.warn(
          `[BES] Skipping handler "${handlerModule.name}" due to missing exports`,
        );
        return;
      }
    ...
    logger.info(`[BES] handlers loaded (${Date.now() - startTimer}ms)`);
  } catch (err) {
    logger.error(
      '[BES] Error loading handlers, check the generated index file:',
      err,
    );
  }
};
